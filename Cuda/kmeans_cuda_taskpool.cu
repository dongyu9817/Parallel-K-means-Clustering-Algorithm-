/**
 * Cuda K-means cluster Algorithm with dynamic task pool
 * Yu Dong (Yu Dong)
 * This is a CUDA-based inequality reinforced k-means algorithm with the implementation of the dynamic task assignment.
 * This algorithm can achieve better load balanced by reducing distance calculations.
 * The basic idea of this algorithm is:
 * 0: Partition original datasets into disjoint subsets of data points, share data points and centroid info with working threads.
 ...
 * 4a. Each node fetches a new task, loads a subset of data points and current membership, and executes a CUDA kernel to find the new membership of all data points in the task.
 * 5: Copy {L1...n} back to host
 * 6: Calculate the mean for each cluster and update {C1...k}
 * 7: A node fetches and processes a new task from the task pool until there are no more tasks.
 */
...
{
    // icd is a k Ã— k matrix storing the distances between every two centroids
    int c1 = 0;
    int c2 = 0;
...
{
    // RID another k Ã— k matrix,where each row is a permutation of 1, 2, Â· Â· Â· k representing the closeness of the distances from Ci to other centroids
...
double kmeans_cuda_triangle_ineq(int *n_points, int clusters, points_t **p_list, centroids_t **c_list, int iterations)
{
    ...
    *c_list = (centroids_t *)malloc(sizeof(centroids_t) * clusters);
    ...
    // labeling n data points are distributed to a total number of GridSize Ã— BlockSize threads, so each thread is responsible for a subset of t = dn/GridSize/BlockSizee
